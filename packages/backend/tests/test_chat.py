...
async def test_chat_no_login_form_completes_immediately() -> None:
    page = {
        ...
        "title": "Home",
        "text": "Welcome",
        "elements": [{"index": 0, "tag": "button", "text": "Go"}],
    }
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as client:
...
async def test_gauss_backend_streams_action_and_done(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        sse_body = (
            ...
            b'data: {"content": "ACTION: {\\"kind\\": \\"fill\\", \\"index\\": 0, \\"value\\": \\"x\\"}\\n", "finish_reason": null}\n\n'
    ...
    actions = [p for p in parsed if p.get("type") == "action"]
    assert len(actions) == 1
    assert actions[0]["action"] == {"kind": "fill", "index": 0, "value": "x"}
...
async def test_gauss_backend_buffers_action_split_across_chunks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    actions = [p for p in parsed if p.get("type") == "action"]
    assert len(actions) == 1
    assert actions[0]["action"] == {"kind": "click", "index": 0}
...
async def test_gauss_backend_multi_turn_contents(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    history = [
        {
            ...
            "actions": [
                {
                    "action": {"kind": "fill", "index": 0, "value": "user@example.com"},
                    "result": {"ok": True},
                }
            ],
            "page": None,
    ...
    expected_model_turn = 'ACTION: {"kind": "fill", "index": 0, "value": "user@example.com"}\nDONE: false'
